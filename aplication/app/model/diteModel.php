...
class DiteModel extends Model
{
	...
	{
    	return $this->db->fetchAll('SELECT * FROM relaceditesponzor AS r , dite AS d WHERE r.diteIdDite = d.idDite AND d.aktivniZaznam = 1 AND d.vystavene = 1 AND d.jmeno LIKE ("%'.$search.'%") GROUP BY d.idDite ORDER BY d.jmeno');
	...
	{
		return $this->db->fetchAll('SELECT d.*, s.jmeno AS jmenoSponzor, sk.nazev AS skolaNazev, sk.idSkola AS skolaId, sk.castka AS castka FROM dite AS d LEFT JOIN (sponzor AS s, relaceditesponzor AS r) ON (d.idDite = r.diteIdDite AND r.sponzorIdSponzor = s.idSponzor) LEFT JOIN (skola as sk) ON (d.skolaIdSkola = idSkola) WHERE d.idDite = '.$id.' GROUP BY d.idDite ORDER BY d.jmeno ');
