...
class DiteModel extends Model
{
	...
	public function zobrazDeti($filter)
	{
		...
		*/
		...
		$this->getDb()->query('CREATE VIEW detiPohled as SELECT d.*, s.jmeno AS jmenoSponzor, sk.nazev AS skolaNazev, sk.idSkola AS skolaId FROM dite AS d, sponzor AS s, skola as sk, relaceditesponzor AS r WHERE (d.idDite = r.diteIdDite AND r.sponzorIdSponzor = s.idSponzor) AND d.skolaIdSkola = idSkola');
